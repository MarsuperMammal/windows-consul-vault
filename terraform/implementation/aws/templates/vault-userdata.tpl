...
(Get-Content "$acrylicConfig" | ForEach-Object { $_ -replace "{{ dns_server }}", "${dns_server}" } ) | Set-Content "$acrylicConfig"
...
(Get-Content C:\etc\vault.d\vault.hcl | ForEach-Object { $_ -replace "{{ datacenter }}", "${datacenter}" } ) | Set-Content C:\etc\vault.d\vault.hcl
